...
public class LectureMysqlDAOImpl implements ILectureDAO {
	@Autowired
	JdbcTemplate jtem;
	final String SELECT_LECTURES_limit = "select * from lecfly_db order by blank  limit?,? where fid = ?";
	...
	public List<LectureVO> selectLectureList(int id, int offset, int limit, int order) {
		return jtem.query(SELECT_LECTURES, BeanPropertyRowMapper.newInstance(LectureVO.class),offset,limit,order , id );
