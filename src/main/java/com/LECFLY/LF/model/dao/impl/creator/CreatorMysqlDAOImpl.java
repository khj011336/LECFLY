...
public class CreatorMysqlDAOImpl implements ICreatorDAO {
	 ...
	 public boolean insertNewCreator(CreatorVO crvo) {
		  int r = jtem.update(INSERT_NEW_CREATOR,crvo.getFid(),crvo.getImgPath(),crvo.getName(),crvo.getNickname(),crvo.getCellPhone()
				 ,crvo.getSNS(),crvo.getInfo(),crvo.getStatus());
		  return r==1;
	 }
	 public boolean insertNewCreator(int fid , String imgPath, String name , String nickname, String cellphone, String sns, String info ,int status) {
	...
	public boolean updateCreator(CreatorVO crVo , int fid) {
		int r= jtem.update(UPDATE_CREATOR,crVo.getImgPath(),crVo.getNickname(),crVo.getCellPhone(),crVo.getSNS(),crVo.getInfo(),crVo.getStatus(),fid);
		return r== 1;
	}
}
